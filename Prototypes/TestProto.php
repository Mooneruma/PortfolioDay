	...
	if(isset($_POST["submit"])){
		...
		*/
		...
		if($valid == true){
			...
			$stmt = $conn->prepare("INSERT INTO student_info_2020 (student_first_name, student_last_name,  student_username, student_password,  student_program, student_portfolio, student_linkedin, student_secondary, student_hometown, student_career_goals, student_hobbies, student_state) 
			VALUES (:student_first_name, :student_last_name,  :student_username, :student_password,  :student_program, :student_portfolio, :student_linkedin, :student_secondary, :student_hometown, :student_career_goals, :student_hobbies, :student_state)");
			...
			$stmt->bindParam(':student_program', $student_program);
			$stmt->bindParam(':student_portfolio', $student_portfolio);
			$stmt->bindParam(':student_linkedin', $student_linkedin);
			$stmt->bindParam(':student_secondary', $student_secondary);
			$stmt->bindParam(':student_hometown', $student_hometown);
			$stmt->bindParam(':student_career_goals', $student_career_goals);
			$stmt->bindParam(':student_hobbies', $student_hobbies);			
			$stmt->bindParam(':student_state', $student_state);
    ...
    grecaptcha.execute('<?php echo SITE_KEY; ?>', {action: 'homepage'}).then(function(token) {
      document.getElementById('g-recaptcha-response').value=token; 
		...
		<?
			...
			<h1><b><a href = "http://kmoonpage.com/Prototypes/StudentLogin.php">Login Page</a></b></h1>
			<?
		}
		?>
		</content>
